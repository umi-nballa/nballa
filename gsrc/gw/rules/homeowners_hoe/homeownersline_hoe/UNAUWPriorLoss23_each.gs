  ...
  override function evaluateRuleCriteria(homeowner : HomeownersLine_HOE) : RuleEvaluationResult {
   ...
   // Claim Amount - HOPriorLosses_Ext.ClaimPayment. ClaimAmount
    ...
    homeowner.HOPriorLosses_Ext?.each( \ elt ->
    {
      ...
      {
        if((elt?.ClaimType!=null && elt?.ClaimType==typekey.ClaimType_Ext.TC_RISK|| elt?.ClaimType!=null && elt?.ClaimType==typekey.ClaimType_Ext.TC_SUBJECT) && (elt1?.LossCause_Ext==typekey.LossCause_Ext.TC_THEFT || elt1?.LossCause_Ext==typekey.LossCause_Ext.TC_THFSC || elt1?.LossCause_Ext==typekey.LossCause_Ext.TC_VMM )
