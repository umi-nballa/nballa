...
abstract class AbstractUnderwriterEvaluator implements UnderwriterEvaluator {
  ...
  private function maybeAddUWIssueForProducer(oldProducerName : String, newProducerName : String,
    ...
    if (oldProducerName != newProducerName) {
      ...
      shortDescBlock = \ -> displaykey.UWIssue.ProducerOrgChange.ShortDesc
      longDescriptionBlock = \ -> displaykey.UWIssue.ProducerOrgChange.LongDesc(newProducerName, newCode)
    } else if (oldCode != newCode) {
      uwIssueCode = "ChangedProducerCode"
      shortDescBlock = \ -> displaykey.UWIssue.ProducerCodeChange.ShortDesc
      longDescriptionBlock = \ -> displaykey.UWIssue.ProducerCodeChange.LongDesc(newProducerName, newCode)
    }
