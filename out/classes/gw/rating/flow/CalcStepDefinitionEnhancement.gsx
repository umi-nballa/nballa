...
enhancement CalcStepDefinitionEnhancement : entity.CalcStepDefinition {
  ...
  function getAllExistingVariableNames(includeComplexVars : boolean) : List<LocalVariable> {
    ...
    for (storeStep in targetVarSteps) {
      ...
      if (includeComplexVars and operandType == MultiFactorVariable) {
        addMultiFactorVariableNamesAndRemoveInvalidVariables(varName, storeStep, varsSoFar)
      }
    }
  ...
  private function addMultiFactorVariableNamesAndRemoveInvalidVariables(varName : String, storeStep : CalcStepDefinition, varsSoFar : LinkedHashMap<String, LocalVariable>)  {
    var rateTable = getStoreStepRTD(storeStep)
    if (rateTable != null) {
  ...
  private function getStoreStepRTD(step: CalcStepDefinition) : RateTableDefinition {
    var rtdAssignmentStep = step
    var rtd =  step.PrimaryOperand.RateTableDefinition
    /*
     * This while loop finds the first step where the variable is declared
     * If the rate table definition is null we will keep searching for the step that has the rate table definition
     * If it isn't a local var or an inscope it will most likely be a constant
     */
    while (rtd == null) {
      if (rtdAssignmentStep.PrimaryOperand.OperandType == TC_LOCALVAR) {
        rtdAssignmentStep =
            rtdAssignmentStep.CalcRoutineDefinition.FilteredOrderedSteps(\step2 ->step2.StoreLocation == rtdAssignmentStep.PrimaryOperand.VariableName)
                .first()
        rtd = rtdAssignmentStep.PrimaryOperand.RateTableDefinition
      } else if (rtdAssignmentStep.PrimaryOperand.OperandType == TC_INSCOPE) {
        rtdAssignmentStep =
            rtdAssignmentStep.CalcRoutineDefinition.FilteredOrderedSteps(\step2 ->step2.InScopeValue == rtdAssignmentStep.PrimaryOperand.InScopeValue)
                .first()
        rtd = rtdAssignmentStep.PrimaryOperand.RateTableDefinition
      } else {
        // when the step operand is of type constant
        return null
      }
    }
    return rtd
