...
class GLRateRoutineExecutor {
  ...
  construct(referenceDate : IReferenceDatePlugin, line : GLLine, ratingLevel : RateBookStatus) {
  ...
  private function getRateBook(refDate : Date) : RateBook {
    ...
    return RateBook.selectRateBook(refDate, _line.Branch.RateAsOfDate, _line.PolicyLine.PatternCode,
      ...
      if (QueryUtils.getRateBooksForLine("GLLine").size() == 0) {
        _logger.error("No Rate Books are loaded for the GL Line")
        throw new RatingException("No Rate Books are loaded for the GL Line")
      } else {
        _logger.error("No Rate Books for GL with " + _ratingLevel.Description + " status matches the filter criteria")
        throw new RatingException("No Rate Books for GL with " + _ratingLevel.Description + " status matches the filter criteria")
      }
    }
  }
