...
Ext.define('gw.Paging', {
  ...
  // reset the LV cell focus if user navigates to a different page in the same LV
  onPagingChange: function() {
    var me = this;
    var grid = Ext.ComponentManager.get(me.gridId);
    if (grid && grid.selModel) {
      grid.selModel.deselect();
    }
  },

  // Add Listeners to pagingChange event from the model so we can handle the LV focus
  getStoreListeners: function() {
    return Ext.apply({pagingChange: this.onPagingChange}, this.callParent(arguments));
  }
});
