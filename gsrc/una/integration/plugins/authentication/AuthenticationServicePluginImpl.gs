...
public class AuthenticationServicePluginImpl implements AuthenticationServicePlugin {
  ...
  override function authenticate(authSource: AuthenticationSource): String {
    ...
    if (StringUtils.isEmpty(source.Username) || StringUtils.isEmpty(source.Password)) {
      LOGGER.warn("Empty username or password provided")
      throw new DisplayableLoginException (displaykey.Integration.LDAP.LoginFailed)
    }
    ...
    if (user.Credential.Locked) {
      ...
      throw new gw.plugin.security.LockedCredentialException()
    }
    if (!user.Credential.Active) {
      LOGGER.warn("User account is inactive for the username: ${source.Username}")
      throw new gw.plugin.security.InactiveUserException()
    }
  ...
  private function authenticateUserAtLDAPServer(source: UserNamePasswordAuthenticationSource) {
    ...
    } catch (ex: DisplayableLoginException) {
      LOGGER.warn(ex.Message)
      throw ex
    } catch (ex: Exception) {
      LOGGER.error("Unexpected Error during Authentication", ex)
      throw new DisplayableLoginException (displaykey.Integration.LDAP.SystemUnavailable)
    }
  }
