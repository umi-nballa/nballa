...
abstract class AbstractBaseAutomaticRateBookLoader {
  ...
  protected function testableRun() {
    ...
    _currentStatus = "Examining rate book XML files in " + DirectoryContainingRateBookXmlFiles.CanonicalPath  // (This status is replaced with another almost instantly.)
    ...
    for (entry in mapOfRateBookCodesToFiles.entrySet()) {
      ...
      for (rateBookPropertiesFromFile in entry.Value) {
        ...
        final var rateBookDescription = "RateBook(code: " + rateBookCode + ", version: " + rateBookEdition + ")"  // (same format as 'entity.RateBook.toString()' method.)
        ...
        final var hasWebSession = true//canLoadRateBooks()

        if (rateBookFromDatabase == null) {
          deleteExistingRatebook = false  // does not exist in database; no delete needed
          loadRatebookFromDisk   = true   // Should load from disk.
          if (hasWebSession) {
            _rfLogger.info("  " + rateBookDescription + " is not in the database.  It will be loaded from disk.")
          } else {
            loadRatebookFromDisk = false  // Do not load, as loading would fail. (...when called form web services or GUnit.)
        ...
        } else { // This Rate Book Edition exists on the disk and in the database.
          ...
          if (rateBookFromDatabase.LastStatusChangeDate.Time < rateBookPropertiesFromFile.LastStatusChangeDate.Time) {
            var reloadThisRateBook = true  // The rate book in the database is older than the one on the disk; we should reload it.
            ...
            if (rateBookFromDatabase.Status.Ordinal == typekey.RateBookStatus.TC_ACTIVE.Ordinal) {
              reloadThisRateBook = false  // We will not delete Active rate books!
            ...
            if (not hasWebSession) {
              reloadThisRateBook = false  // Do not load, as loading would fail. (...when called form web services or GUnit.)
        ...
        if (hasWebSession) {
          ...
          if (deleteExistingRatebook and rateBookFromDatabase != null) {
            ...
            rateBookFromDatabase = null  // This 'entity.RateBook' object is no longer valid.
          }
    ...
    if (DirectoryContainingRateBookXmlFiles.exists()) {
      for (childFile in DirectoryContainingRateBookXmlFiles.Children) {
        if (childFile.Name.endsWith(".xml")) {
          ...
          if (rateBookProperties != null) {                              // (Silently discard xml files that are not rate book exports.)
  ...
  public function readRateBookProperties(final javaFile : File) : RateBookProperties {
    ...
    final var saxContentHandler = new RateBookXmlSaxContentHandler(javaFile)  // (Hint: Our Rate Book specific parsing logic is in this class.)
    ...
    try {
       ...
       */
      return ex.RateBookProperties    // Normal successful return point.

    } catch (saxEx : SAXException) {
      if (saxEx.Cause typeis NonRateBookGuidewireImportFileException) {
        return null  // This is a Guidewire import file for some other tables. IE: It's NOT a rate book export file.
      } else {
