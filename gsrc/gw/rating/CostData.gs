...
abstract class CostData<S extends entity.Cost, V extends PolicyLine> extends CostDataBase implements WorksheetEntryContainer{
  ...
  private class LazyFXConversion extends LockingLazyVar<MonetaryAmount> {
    ...
    function scaleWithLogging(logger: WorksheetLogger) : MonetaryAmount {
      ...
      var unscaledAmt = logger.let(displaykey.Web.Policy.RatingWorksheet.ConvertedTermAmount,  \ ->
       logger.Term.vr(displaykey.Web.Policy.RatingWorksheet.TermAmount, unconverted) *
            logger.Multiplication.vr(displaykey.Web.Policy.RatingWorksheet.ForeignExchangeRate, _policyFXRate.Rate))

      var converted = unconverted.ofCurrency(Currency).convertAmount(_policyFXRate)
      assert unscaledAmt == converted.Amount

      var scaledAmt = logger.let(displaykey.Web.Policy.RatingWorksheet.ConvertedTermAmountRounded,
          \ -> logger.Term.fn(gw.rating.flow.CalcRoutineRoundingMethod, "setScale", "Rounding",
              \ ->  gw.rating.flow.CalcRoutineRoundingMethod.setScale(  logger.arg("Value", \-> logger.Term.vr(displaykey.Web.Policy.RatingWorksheet.ConvertedTermAmount, converted)),
