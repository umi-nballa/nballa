package gwservices.pc.dm.plugin
...
@Export
class DMPolicyNumGenPlugin implements IPolicyNumGenPlugin {
  override function genNewPeriodPolicyNumber(policyPeriod: PolicyPeriod): String {
    if (policyPeriod.Job.MigrationJob and policyPeriod.PolicyNumber.HasContent) {
  ...
  private function constructNextPolicyNumber(sInitialPolicyNum: String, moduleNumber: Integer): String {
    ...
    if (moduleNumber < 10) {
      sReturnVal = sReturnVal + "0"
      // pad Module # to 2 digits
    }
