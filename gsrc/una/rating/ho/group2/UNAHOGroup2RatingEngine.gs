...
class UNAHOGroup2RatingEngine extends UNAHORatingEngine_HOE<HomeownersLine_HOE> {
  ...
  override function rateDwellingCoverages(dwellingCov: DwellingCov_HOE, dateRange: DateRange) {
    switch (typeof dwellingCov) {
      ...
      case HODW_Personal_Property_HOE:
        if(PolicyLine.HOPolicyType != typekey.HOPolicyType_HOE.TC_HO4 and _dwellingRatingInfo.PersonalPropertyIncreasedLimit > 0 ){
  ...
  */
  ...
  function rateEarthquakeCoverage(dwellingCov : HODW_Earthquake_HOE, dateRange : DateRange){
    ...
    var costData = HOCreateCostDataUtil.createCostDataForDwellingCoverage(dwellingCov, dateRange, HORateRoutineNames.EQ_COVERAGE_RATE_ROUTINE, RateCache, PolicyLine, rateRoutineParameterMap, Executor, this.NumDaysInCoverageRatedTerm)
    if (costData != null)
      addCost(costData)
    if (_logger.DebugEnabled)
  ...
  function ratePersonalPropertyReplacementCost(dateRange: DateRange) {
    ...
    if (costData != null)
      addCost(costData)
    _hoRatingInfo.PersonalPropertyReplacementCost = costData.ActualTermAmount
  ...
  private function updateTotalBasePremium() {
    _hoRatingInfo.TotalBasePremium = _hoRatingInfo.AdjustedBaseClassPremium + _hoRatingInfo.SuperiorConstructionDiscount + _hoRatingInfo.TownhouseOrRowhouseSurcharge + _hoRatingInfo.ProtectiveDevicesDiscount +
                                     ...
                                     _hoRatingInfo.DiscountAdjustment + _hoRatingInfo.SeasonalSecondaryResidenceSurcharge + _hoRatingInfo.PersonalPropertyReplacementCost
