...
enhancement PolicyPeriodReinsuranceEnhancement : entity.PolicyPeriod {
  ...
  function regenerateRisks() : PolicyPeriod {
    ...
    gw.transaction.Transaction.runWithNewBundle(\ bundle -> {
      ...
      var reinsurables = Query.make(Reinsurable).compare("BranchValue", Relop.Equals, periodInBundle).select()
      var reinsurablesToProcess : Map<String, Reinsurable> = new HashMap<String,Reinsurable>()
      reinsurables.each(\ r -> {
          var reinsurable = r.getSlice(r.EffectiveDate)
          reinsurablesToProcess.put(reinsurable.RiskNumber,reinsurable)
      })

      reinsurablesToProcess.values().each(\ r -> {
        r = bundle.add(r)
        plugin.reattachRisk(r)
      })
    })
  ...
  private function removeInactiveRiskNumbers(plugin : IReinsurancePlugin)  {
    ...
    for (slice in this.OOSSlices) {
      activeRiskNumbers.addAll((slice.AllReinsurables*.RiskNumber) as Collection<String>)
    }
