...
class BP7ClassSchedCondImpl extends AbstractScheduleWithDescriptionImpl<BP7ClassSchedCond> {

  construct(delegateOwner : BP7ClassSchedCond) {
    super(delegateOwner)
  }
  ...
  override function createAndAddScheduledItem() : ScheduledItem {
    var scheduledItem = new BP7ClassSchedCondItem(Owner.Branch)
  ...
  override function removeScheduledItem(item : ScheduledItem) {
    Owner.removeFromScheduledItems(item as BP7ClassSchedCondItem)
  ...
  override property get CurrentAndFutureScheduledItems() : KeyableBean[] {
    ...
    Owner.Branch.OOSSlices
      ...
      .each(\ period -> {
        var matchingSlicedScheduleCond = period.BP7Line.ConditionsFromCoverable.firstWhere(\ c -> c.FixedId == Owner.FixedId) as BP7ClassSchedCond
