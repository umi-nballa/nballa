...
enhancement CalcRoutineParameterSetEnhancement : entity.CalcRoutineParameterSet {
  ...
  static property get ParamSetsReferencedForRoutinesInPromotedBook() : java.util.Set<String> {
    var promotedRateBooksQuery = Query.make(RateBook).compare("Status", NotEquals, RateBookStatus.TC_DRAFT)

    var routineDefQuery = Query.make(CalcRoutineDefinition)
    routineDefQuery.join(RateBookCalcRoutine, "CalcRoutineDefinition").subselect("RateBook",CompareIn,promotedRateBooksQuery,"ID")

    var paramSetQuery = Query.make(CalcRoutineParameterSet)
    return (paramSetQuery.subselect("ID", CompareIn, routineDefQuery, "ParameterSet").select().map(\crps -> crps.Code).toSet())
  }

  static property get ParamSetsReferencedInRateTableDefinitions() : java.util.Set<String> {
    var paramSetQuery = Query.make(CalcRoutineParameterSet)
    return paramSetQuery.join(RateTableArgumentSourceSet, "CalcRoutineParameterSet").select().map(\crps -> crps.Code).toSet()
  }
  ...
  static property get ParamSetsReferencedInRateTableDefinitionsInPromotedBooks() : java.util.Set<String> {
    ...
    return (paramSetQuery.select().map(\crps -> crps.Code).toSet())
  }
