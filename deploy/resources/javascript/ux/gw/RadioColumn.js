...
Ext.define('Ext.ux.gw.RadioColumn', {
  ...
  processEvent: function(type, view, cell, recordIndex, cellIndex, e, record, row) {
    ...
    // onTarget is that we actually click on the radio button itself, not the white space around it inside the cell
    onTarget =  mousedown && !(Ext.fly(e.target).query('[class^=x-grid3-radio-col]').length > 0);

    // boolean cell cannot be disabled
    // only toggle if the radio button is clicked on a space key is pressed with the cell in focus
    if (!me.disabled && (onTarget || (key && e.getKey() == e.SPACE))) {
    ...
    } else if (!me.disabled && (key && e.getKey() == e.ENTER)) {
      // forward the Enter key to handle default action
