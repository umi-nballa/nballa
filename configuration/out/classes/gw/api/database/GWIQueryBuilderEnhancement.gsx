...
enhancement GWIQueryBuilderEnhancement<QT extends gw.pl.persistence.core.Bean> : IQueryBuilder<QT>
{
   ...
   *
   * Adding additional columns will cause the join to be specified as a select.  The additional columns can
   * contain SQL functions allowing you to do an implicit group by on the remaining columns.
   *
   * @param propertyOnThisEntity the column to join through
   * @param otherEntity the type for the table to be joined to
   * @param propertyOnOtherEntity the column in the other table to join through
   * @return this.  Used for builder pattern
   */
  function join(propertyOnThisEntity : String, otherEntity : IType, propertyOnOtherEntity : String) : Table<QT> {
    if (propertyOnThisEntity == null) {
      throw new IllegalArgumentException("propertyOnThisEntity cannot be null")
    }
    if (otherEntity == null) {
      throw new IllegalArgumentException("otherEntity cannot be null")
    }
    if (propertyOnOtherEntity == null) {
      throw new IllegalArgumentException("propertyOnOtherEntity be null")
    }
    return this.join(PropertyResolver.getProperty(this.EntityType, propertyOnThisEntity), PropertyResolver.getProperty(otherEntity, propertyOnOtherEntity))
  }
