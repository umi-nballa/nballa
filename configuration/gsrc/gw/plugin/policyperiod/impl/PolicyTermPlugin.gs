...
class PolicyTermPlugin implements IPolicyTermPlugin {
  ...
  override function calculatePeriodEnd( effDate : Date, term : TermType, policyPeriod : PolicyPeriod ): Date {
    ...
    switch (term) {
      case TermType.TC_ANNUAL:
        ...
        break
      case TermType.TC_THREEYEAR:
        futureDate = effDate.addYears(3)
        break
  ...
  override function calculatePolicyTerm(policyPeriod : PolicyPeriod, periodStart: Date, periodEnd: Date): TermType {
    ...
    if (isAnnualTerm(policyPeriod, terms, periodStart, periodEnd)) {
      return annual
    } else if (isThreeYearTerm(policyPeriod, terms, periodStart, periodEnd)) {
      return TermType.TC_THREEYEAR
  ...
  private function isThreeYearTerm(policyPeriod : PolicyPeriod, terms : List<typekey.TermType>, periodStart: Date, periodEnd: Date): boolean {    
    var maxAdditionalDays = 0
    for (line in policyPeriod.Lines) {
      maxAdditionalDays = Math.max(maxAdditionalDays, (line.AdditionalDaysInAnnualTerm * 3))
    }
    
    if (maxAdditionalDays != 0) {
      var minExpDate = calculatePeriodEnd(periodStart, TermType.TC_THREEYEAR, policyPeriod)    
      var maxExpDate = calculatePeriodEnd(periodStart, TermType.TC_THREEYEAR, policyPeriod).addDays(maxAdditionalDays)   
      if (not periodEnd.before(minExpDate) and not periodEnd.after(maxExpDate)) {
        return true
      }
    }
    
    return isTermType(terms, TermType.TC_THREEYEAR, periodStart, periodEnd, policyPeriod)
  }
