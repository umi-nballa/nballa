...
enhancement CalcStepDefinitionOperandEnhancement : entity.CalcStepDefinitionOperand {
  ...
  property get SortedMatchOps() : List<RateTableMatchOp> {
    if (this.RateTableDefinition.ArgumentSourceSets.IsEmpty || this.ArgumentSourceSetCode == null){
      return this.RateTableDefinition.SortedMatchOps
    }
    var sourceSetMatchOps = ArgumentSourceSets.firstWhere( \ elt -> elt.Code == this.ArgumentSourceSetCode).RateTableArgumentSources*.Parameter
    return this.RateTableDefinition.SortedMatchOps.where( \ elt -> sourceSetMatchOps.contains(elt))
  }

  function resetArgumentSources() {
    var args = this.ArgumentSources
    args.each(\argSrc-> this.removeFromArgumentSources(argSrc))
    SortedMatchOps.each(\ matchOp -> {
      this.addToArgumentSources(new CalcStepDefinitionArgument() {
  ...
  function updateArgumentSources() {
    var sortedMatchOpsNames = SortedMatchOps.map( \ elt -> elt.Name)
    var args = this.ArgumentSources
    for (arg in args){
      if (!sortedMatchOpsNames.contains(arg.Parameter)){
        this.removeFromArgumentSources(arg)
      } else {
        sortedMatchOpsNames.remove(arg.Parameter)
      }
    }

    sortedMatchOpsNames.each(\ name -> {
      this.addToArgumentSources(new CalcStepDefinitionArgument() {
          :Parameter = name
      })
    })
  ...
  property get OperandName() : String {
    ...
    switch (this.OperandType) {
      ...
      case TC_RATETABLE:
        ...
        if (this.RateTableDefinition.ArgumentSourceSets.Count > 1 and this.ArgumentSourceSetCode != null) {
  ...
  property get IsMultiFactorRateTableLookup() : boolean {
    return this.OperandType == TC_RATETABLE and this.ReturnFactorColumns.Count > 1
  }

  property get IsMultiFactorVariable() : boolean {
    if (this.VariableFieldName.HasContent and
        (this.VariableFieldName == RateRoutineRateTablePopupHelper.AllFactorsCode or this.ReturnFactorColumns.Count > 1)) {
      return true
    }
    return false
  }
  ...
  @SuppressWarnings("all") // matchOps : Set<String> is necessary
  function mismatchingRateTableMatchops() : String[] {
    if (this.OperandType == TC_RATETABLE and this.TableCode.NotBlank and this.ArgumentSourceSetCode.NotBlank) {
      var rateTableDef = this.RateTableDefinition
      var argumentSourceSet = rateTableDef.ArgumentSourceSets.singleWhere(\ argSourceSet -> argSourceSet.Code == this.ArgumentSourceSetCode)
      var paramsInArgSourceSet = argumentSourceSet.RateTableArgumentSources*.Parameter*.Name.toSet()
      var paramsUsedInLookup = this.ArgumentSources*.Parameter.toSet()
      var extraParamsInLookup = paramsUsedInLookup.subtract(paramsInArgSourceSet)
      var missingParamsFromLookup = paramsInArgSourceSet.subtract(paramsUsedInLookup)
      var misMatches : Set<String> = {}
      if (extraParamsInLookup.HasElements) {
        misMatches.add(displaykey.Validation.Rating.RateRoutineDefinition.BadRateTableParameters(extraParamsInLookup.join(", ")))
      }
      if (missingParamsFromLookup.HasElements) {
        var rRoutines = rateTableDef.ReferencingRateRoutines.firstWhere( \ elt -> elt.isIncludedInPromotedRateBook())
        if(rRoutines == null){
          misMatches.add(displaykey.Validation.Rating.RateRoutineDefinition.ExtraRateTableParameters(missingParamsFromLookup.join(", ")))
        }
      }
