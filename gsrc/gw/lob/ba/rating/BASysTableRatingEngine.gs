...
class BASysTableRatingEngine extends AbstractRatingEngine<BusinessAutoLine> {
  ...
  private function rateCancellationShortRatePenalty(lineVersion : BusinessAutoLine) {
    ...
    if ( lineVersion.Branch.RefundCalcMethod == "shortrate" ) { // context.CancelMethod returns null if the period is not canceled
      var basesMap = getAmountSumByState( getCostsUpToRatedOrder( "CancelShortRatePenalty" ).where( \ cost -> cost.ProrationMethod != TC_FLAT) )
      for ( st in basesMap.Keys ) {
        ...
        if ( subtotal != 0 ) {
          ...
          var termSubTotal =  getCostsUpToRatedOrder( "CancelShortRatePenalty" ).where( \ cost -> cost.ProrationMethod != TC_FLAT).where( \ c -> c.JurisdictionArg.State == st ).sum(\ c -> p.prorate(lineVersion.Branch.PeriodStart, p.findEndOfRatedTerm(lineVersion.Branch.PeriodStart, c.NumDaysInRatedTerm),
