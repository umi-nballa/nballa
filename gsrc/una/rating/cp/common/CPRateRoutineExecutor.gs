...
class CPRateRoutineExecutor {
  ...
  private function getRateBook(refDate : Date) : RateBook {
    ...
    return RateBook.selectRateBook(refDate, _line.Branch.RateAsOfDate, _line.PolicyLine.PatternCode,
      ...
      if (QueryUtils.getRateBooksForLine("CPLine").size() == 0) {
        _logger.error("No Rate Books are loaded for the CP Line")
        throw new RatingException("No Rate Books are loaded for the CP Line")
      } else {
        _logger.error("No Rate Books for CP with " + _ratingLevel.Description + " status matches the filter criteria")
        throw new RatingException("No Rate Books for CP with " + _ratingLevel.Description + " status matches the filter criteria")
      }
    }
  }
