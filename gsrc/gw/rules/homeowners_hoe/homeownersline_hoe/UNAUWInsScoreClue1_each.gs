 ...
 */
class UNAUWInsScoreClue1_each implements IRuleCondition<HomeownersLine_HOE>{
  override function evaluateRuleCriteria(homeowner : HomeownersLine_HOE) : RuleEvaluationResult {
   ...
   //InScore<626 and 1 prior "Water", "Appliance", "Mold"
    ...
    if(homeowner.AssociatedPolicyPeriod?.CreditInfoExt?.CreditReport?.CreditScore<626)
      {
        homeowner.HOPriorLosses_Ext.each( \ elt ->
        {
          ...
          {
            if(elt1.LossCause_Ext == typekey.LossCause_Ext.TC_WATER || elt1.LossCause_Ext==typekey.LossCause_Ext.TC_APPLIANCERELATEDWATER || elt1.LossCause_Ext==typekey.LossCause_Ext.TC_MOLD)
