  ...
  public static function fillBasePropertyTotalAmount(dto: QuoteDTO, pp: PolicyPeriod) {
    if (pp.TotalCostRPT != null) {
      ...
      if (pp.HomeownersLine_HOEExists){
        dto.Lobs = new QuoteLobDataDTO()

        if (pp.HomeownersLine_HOE.Dwelling.HODW_FloodCoverage_HOE_ExtExists) {
          dto.Lobs.Homeowners = new HOPremiumCostsDTO (){: FloodPremium = pp.HomeownersLine_HOE.Dwelling.HODW_FloodCoverage_HOE_Ext.Cost.ActualAmount.Amount }
        } else {
