 ...
 */
class UNAUWPriorLoss42_each implements IRuleCondition<HomeownersLine_HOE>{
  override function evaluateRuleCriteria(homeowner : HomeownersLine_HOE) : RuleEvaluationResult {

    //2 or more losses (UNA Claims + CLUE or Agent Entered) in last 3 years, regardless of amount paid,
    // including Equipment Breakdown losses.   1 of thes losses must be a UNA loss.
         ...
         homeowner.HOPriorLosses_Ext.each( \ elt ->
         {
             ...
             //return RuleEvaluationResult.execute()
           {
             elt.ClaimPayment.each( \ elt1 ->
             {
               ...
               if(DateUtil.addYears(elt.ClaimDate as java.util.Date,3)>new java.util.Date() )
               {
                 clmcount++
